...
>如果对如何编写 Nginx 配置文件不是很熟悉，可以使用 https://nginxconfig.io/ 完成配置

HFish 分别使用了 `4433` 和 `4434` 端口。其中 `4433` 用于对外提供 Web 服务和部分 API 接口；`4434` 端口用于节点通讯。因此，Nginx 需同时代理这两个端口，方可提供服务。
